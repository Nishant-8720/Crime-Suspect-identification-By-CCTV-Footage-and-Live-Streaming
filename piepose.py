...
def poseRes():
	df = pd.read_csv('isPose.csv')
	s = df['probabilities'].value_counts().rename('Total_Pose')
	s.to_csv('test_Pose.csv')
	df1 = pd.read_csv('test_Pose.csv')
	df1.columns.values[0] = 'index'
	df1.to_csv('test_Pose.csv',index=False)
	df2 =  pd.read_csv('test_Pose.csv')
	# df.to_frame()
